...
# What does it do?
You give it a search query (e.g. ADHD AND insomnia) and optionally a publication year. It returns a list of matching papers with everything regarding the found papers, and a direct link to read it for free.

# Input 
query: required, your search query. Supports full PubMed syntax.
publication year: optional, filter by publication year.
ncbiApiKey: optional, free API key from NCBI. Increases rate limits. Obteinable at ncbi.nlm.nih.gov/account 

# Output 
Each result contains title, year, authors, journal, publicatoin date, pmcId, and the url. 
